...
//reduce the array: takes all numbers in the array and adds them.
...
//apparently do a reduce-> determine value of spaces
...
//reduce problem: takes in an array of booleans and returns the number of 'true' values.
...
//string building problem: takes a paragraph and replaces periods with exclamation points.
function makeFriendly(para) {
  let period = ".";
  let exclamation = "!";
  for (const value of para) {
    const result = para.split(period).join(exclamation);
    return result;
  }
}

// given an array of numbers, returns an array where each is cubed.
...
// given an array of strings, return array where string has appended to it the corresponding animal's noise.
//Fido, Rolph and Maisie all say "Woof!"
//Garfield and heathcliff bot say "Meow!"
//Barnie and Sharp Tooth both say "ROWR!"
function addNoises(arr) {
  let output = [];
  const bark = ' says "Woof!"';
  const meow = ' says "Meow!"';
  const rawr = ' says "ROWR."';
  for (const value of arr) {
    if (value === "Fido" || value === "Rolph" || value === "Maisie") {
      output.push(value + bark);
    }
    if (value === "Garfield" || value === "Heathcliff") {
      output.push(value + meow);
    }
    if (value === "Barnie" || value === "Sharp Tooth") {
      output.push(value + rawr);
    }
  }
  return output;
}

//given an array of strings with names and gender, return an array of only those strings where the gender(last char) is F.
...
//given an array of numbers, returns an array of only those numbers that are integers.
...
describe("womenOnly", () => {
  it(`returns an array of only those strings in the given array where the last character is "F"`, () => {
    const people1 = ["Colin - M", "Mesuara - F", "Ginny - F", "Anthony - M"];

    const answer1 = ["Mesuara - F", "Ginny - F"];

    const people2 = ["Ted - M", "Melissa - F", "Biff - M", "Spiffs - F"];

    const answer2 = ["Melissa - F", "Spiffs - F"];
